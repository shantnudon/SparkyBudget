...
function aggregateData(transaction_data, selectedOption) {
  ...
  for (var i = 0; i < transaction_data.length; i++) {
    var selectedField =
      transaction_data[i][getIndexForSelectedOption(selectedOption)];
    var transactionAmount = parseFloat(transaction_data[i][9]); // Updated index from 8 to 9
    var key = selectedField;
    if (aggregatedData[key]) {
      aggregatedData[key].count += 1;
      aggregatedData[key].totalAmount += isNaN(transactionAmount)
        ? 0
        : transactionAmount;
    } else {
      aggregatedData[key] = {
  ...
  for (var key in aggregatedData) {
    if (aggregatedData.hasOwnProperty(key)) {
      aggregatedData[key].totalAmount = Math.round(
        Math.abs(aggregatedData[key].totalAmount)
      );
    }
  }
...
function getIndexForSelectedOption(selectedOption) {
  switch (selectedOption) {
    ...
    case "FormattedMonth":
      return 2;
    case "Payee":
      return 6; // Updated index from 5 to 6
    case "Subcategory":
      return 7; // Updated index from 6 to 7
    default:
      return 7; // Updated default index from 6 to 7
  }
}
...
function updateChart(aggregatedData) {
  ...
  for (var i = 0; i < categories.length; i++) {
    ...
    if (isNaN(filteredData[category])) {
      console.error(
        "NaN value found in totalAmount for category:",
        category,
  ...
  d3.selectAll(
    "#bar_chart .c3-axis-x-label, #bar_chart .c3-axis-y-label"
  ).style("fill", textColor);
  d3.select("#bar_chart .c3-title").style("fill", textColor);
  d3.selectAll(
    "#bar_chart .c3-axis-x .tick text, #bar_chart .c3-axis-y .tick text"
  ).style("fill", textColor);
}
...
function renderLineChart(lineChartData) {
  ...
  const gridColor = isLightTheme
    ? "rgba(0, 0, 0, 0.1)"
    : "rgba(255, 255, 255, 0.1)"; // Use light gray for light theme, white for dark
...
$(document).ready(function () {
          ...
          $.ajax({
            ...
            success: function (response) {
              subcategoryDropdown.html(
                '<select class="subcategory-select"></select>'
              );
              var subcategorySelect = subcategoryDropdown.find(
                ".subcategory-select"
              );
                ...
                $.ajax({
                  ...
                  error: function (error) {
                    ...
                    $("<div></div>")
                      .appendTo("body")
                      .html(
                        "<div><h6>Error updating subcategory: " +
                          error +
                          "</h6></div>"
                      )
                      .dialog({
  ...
  $(".transaction-table-mobile-container").on(
    "click",
      ...
      if (subcategoryDropdown.is(":visible")) {
        $.ajax({
          ...
          success: function (response) {
            ...
            subcategorySelect.on("select2:select", function (e) {
              var updatedSubcategory = e.params.data.text;
              var transactionId = card
                .find(".transaction-key span:last")
                .text();
                ...
                <div class="transaction_details_card">
                    <div class="transaction_details_header">
                        <div class="transaction_details_info">
                            <span class="transaction_details_payee">${
                              transaction[6]
                            }</span> {# Payee is now at index 6 #}
                            <span class="transaction_details_date">${
                              transaction[3]
                            }</span> {# Transaction Date is still at index 3 #}
                        </div>
                        <div class="transaction_details_amount_container">
                            <span class="transaction_details_amount">$${transaction[9].toFixed(
                              2
                            )}</span> {# Amount is now at index 9 #}
                    ...
                    <div class="transaction_details_body" style="display: none;">
                        ...
                        <div class="transaction_details_field transaction-key" style="display: none;">
                            ...
                            <span>${
                              transaction[8]
                            }</span> {# Transaction Key is now at index 8 #}
                        </div>
                        <div class="transaction_details_field">
...
$(document).ready(function () {
  ...
  $("#closeTransactionPopupButton, #cancelTransactionButton").on(
    "click",
    function () {
...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    error: function (error) {
      console.error(
        "Error fetching subcategories for split transaction:",
        error
      );
    },
  });
  ...
  $("#transactionTable").on("click", ".split-button", function () {
    ...
    const transactionAmount = parseFloat(
      row
        ...
        .text()
        .replace(/[^0-9.-]+/g, "")
    ); // Updated nth-child from 9 to 10
...
$(".transaction-table-mobile-container").on(
  ...
  function () {
    ...
    const transactionAmountText = card
      .find(".transaction_details_amount")
      .text();
    const transactionAmount = parseFloat(
...
function renderIncomeExpenseChart(chartData) {
  const ctx = document
    .getElementById("incomeExpenseChartCanvas")
    .getContext("2d");
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          fill: false,
          datalabels: {
            // Data labels for Budget line (explicitly disabled)
            display: false,
          },
        },
      ],
...
function renderNetCashFlowChart(chartData) {
  const ctx = document
    .getElementById("netCashFlowChartCanvas")
    .getContext("2d");
  ...
  const netCashFlowData = chartData.map(
    (item, index) => item.Income - item.ExpenseAmount
  ...
  const netCashFlowColors = netCashFlowData.map((value) =>
      ...
      datasets: [
        {
          ...
          datalabels: {
            display: function (context) {
              // Only show datalabels on desktop for bars
              return (
                context.dataset.label === "Income" && window.innerWidth > 768
              );
            },
            anchor: "end",
            align: "top",
        ...
        },
        {
          ...
          datalabels: {
            display: function (context) {
              // Only show datalabels on desktop for bars
              return (
                context.dataset.label === "Expenses" && window.innerWidth > 768
              );
            },
            anchor: "end",
...
function renderSpendingTrendChart(chartData) {
  const ctx = document
    .getElementById("spendingTrendChartCanvas")
    .getContext("2d");

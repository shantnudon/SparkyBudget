$(document).ready(function () {
  ...
  const filtersPickerContainer = document.querySelector(
    ".filters .month-year-picker-container"
  );
  ...
  filtersPicker.querySelectorAll(".month-picker button").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      filtersPicker
        .querySelectorAll(".month-picker button")
        .forEach((button) => {
  ...
  $(document).on("click", ".split-button", function () {
    ...
    const transactionAmountText = card
      .find(".budget_transaction_details_amount")
      .text();
    const transactionAmount = parseFloat(
      transactionAmountText.replace(/[^0-9.-]+/g, "")
  ...
  $(document).on("submit", "#splitTransactionForm", function (e) {
    ...
    const transactionAmount = $("#splitTransactionPopup").data(
      "transactionAmount"
    );
    ...
    if (
      ...
      isNaN(splitAmount) ||
      splitAmount <= 0 ||
      Math.abs(splitAmount) >= Math.abs(transactionAmount)
    ) {
      alert(
    ...
    const adjustedSplitAmount =
      transactionAmount < 0 ? -Math.abs(splitAmount) : Math.abs(splitAmount);
...
function initializeSplitPopupHandlersAndDropdown() {
  ...
  $("#splitTransactionCloseButton, #splitTransactionCancelButton").on(
    "click",
    function () {
...
function balanceDetailsTableToggleVisibility() {
  ...
  var isHidden = balanceDetailsTable.classList.contains(
    "hidden-balance-details"
  );
...
function updateBudgetSummaryChartFromDropdown() {
  ...
  if (!calendarValue) {
    console.error(
      "Calendar picker value is empty. Cannot update budget summary chart."
    );
    return;
  }
...
function updateBudgetSummaryChart(transactionYear, transactionMonth) {
  console.log(
    "Updating budget summary chart for year:",
    transactionYear,
    "and month:",
    transactionMonth
  );
  ...
  var SortAscDesc =
    document.getElementById("SortAscDesc").value.toLowerCase() === "true"; // Convert to boolean

  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      // Include the rendered budget summary chart HTML here
      document.getElementById("budgetSummaryChart").innerHTML =
        xhr.responseText;
    }
  };
...
function showTransactionDetails(event, selectedSubcategory, spentAmount) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      header.textContent =
        selectedSubcategory + " Expenses: " + usdFormatter.format(spentAmount);
      ...
      var tableContainer = tempDiv.querySelector(
        ".sb-table-container.budget-transaction-container"
      );
  ...
  xhr.open(
    "GET",
    "/get_budget_transaction_details?year=" +
      transactionYear +
      "&month=" +
      transactionMonth +
      "&subcategory=" +
...
function editBudget(event, subcategory, currentBudget) {
  ...
  function handleUpdate() {
    ...
    if (Number.isNaN(budgetAmount) || budgetAmount === 0) {
      const resp = confirm(
        "Invalid budget amount. Would you like to go back and fix or cancel?"
      );
      if (resp) {
        setTimeout(() => {
          inputElement.focus();
...
function addBudget() {
  ...
  console.log(
    "Adding new budget: subcategory is ",
    subCategory,
    " year:",
    transactionYear,
    " month:",
...
(function () {
  ...
  const addBudgetForm = document.getElementById("addBudgetForm");
  const monthSelectContainer = addBudgetForm.querySelector(
  ...
  if (
    ((isSafari && !hasTouchScreen) || (isFirefox && !hasTouchScreen)) &&
    addBudgetForm &&
    monthInputField &&
...
function dailybalanceDetailsToggleVisibility() {
  ...
  dailyBalanceContainer.style.display =
...
$(document).ready(function () {
  ...
  $(".dailyBalance_account-type-btn").on("click", function () {
    ...
    if (selectedAccountTypes.includes(accountType)) {
      selectedAccountTypes = selectedAccountTypes.filter(
  ...
  function updateAccountNameFilter(accountTypes) {
    ...
    accountNames.forEach((accountName) => {
      $("#dailyBalance_accountNameFilter").append(
        new Option(accountName, accountName)
      );
    });
  ...
  function aggregateData(selectedAccountTypes, selectedAccountNames) {
    ...
    if (selectedAccountTypes.length === 0) {
      selectedAccountTypes = [
        ...new Set(daily_balance_data.map((data) => data[0])),
      ];
    }

    var filteredData = daily_balance_data.filter(
      (data) =>
        (selectedAccountTypes.length === 0 ||
          selectedAccountTypes.includes(data[0])) &&
        (selectedAccountNames.length === 0 ||
          selectedAccountNames.includes(data[1]))
    );
  ...
  function updateChart() {
    ...
    if (dailyBalanceChart && dailyBalanceChart.data) {
      ...
      const latestBalance =
        aggregatedData.data[aggregatedData.data.length - 1] || 0;
      document.querySelector(
        ".chart-card .views .number"
      ).textContent = `$${latestBalance.toLocaleString()}`;
...
// A function that will get the current theme from localStorage
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      document.getElementById(
        "incomeTotal"
      ).textContent = `Income - ${USDollar.format(income)}`;
      document.getElementById(
        "budgetTotal"
      ).textContent = `Budget - ${USDollar.format(budget)}`;
      ...
      const gradientSpent = chartContextBudget.createLinearGradient(
        0,
        0,
        0,
        400
      ...
      const gradientRemainingBudget = chartContextBudget.createLinearGradient(
        0,
        0,
        0,
        400
          ...
          options: {
            ...
            plugins: {
              legend: {
                ...
                labels: {
                  color:
                    getCurrentTheme() === "light-theme" ? "#000000" : "#d6dadf",
                  boxWidth: 20,
                  padding: 10,
                  font: {
                    family:
              ...
              datalabels: {
                ...
                formatter: (value, context) =>
                  context.dataIndex === 0 ? "" : `$${value.toLocaleString()}`,
                font: {
                  weight: "normal",
                  family:
                    "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif",
      ...
      if (!ctxIncome) {
        console.error(
          'Canvas element with ID "incomeVsBudgetPieChart" not found.'
        );
        return;
      }
      const chartContextIncome = ctxIncome.getContext("2d");

      const gradientBudget = chartContextIncome.createLinearGradient(
        0,
        0,
        0,
        400
      ...
      const gradientRemainingIncome = chartContextIncome.createLinearGradient(
        0,
        0,
        0,
        400
          ...
          options: {
            ...
            plugins: {
              legend: {
                position: "top",
                labels: {
                  color: getCurrentTheme() === 'light-theme' ? '#000000' : '#d6dadf',
                  boxWidth: 20,
                  padding: 10,
                  font: {
                    family:
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      console.error(
        "Error fetching data for pie charts:",
        textStatus,
        errorThrown
      );
...
function updateSubcategory(transactionKey, event) {
  ...
  console.log(
    "Inside updateSubcategory: transactionKey ",
    transactionKey,
    "updatedSubcategory :",
    updatedSubcategory
  );
...
function budget_transaction_details_updateSubcategory(transactionKey, event) {
  var updatedSubcategory = $("#subcategorySelectMobile").select2("data")[0]
    .text;
  console.log(
    "Inside budget_transaction_details_updateSubcategory: transactionKey ",
    transactionKey,
    "updatedSubcategory :",
    updatedSubcategory
  );
